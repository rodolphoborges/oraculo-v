 ...
 *
 * 3 Personas:
 *   1. ADVOGADO   — Defende o jogador pela perspectiva do time aliado
 *   2. PROMOTOR   — Acusa pela perspectiva do time rival
 *   3. ÁRBITRO    — Julga qual análise é superior, sintetiza veredito, evolui base de conhecimento
 */
...
const SYSTEM_ADVOCATE = [
    'Você é o ADVOGADO TÁTICO do Tribunal do Protocolo V.',
    'Sua função é analisar a performance do jogador PELA PERSPECTIVA DO TIME ALIADO.',
    'Defenda a execução quando os dados suportarem. Identifique onde aliados falharam em dar suporte.',
    'NÃO seja cego — se o jogador falhou mesmo com suporte, reconheça.',
    'Responda APENAS em JSON válido. NUNCA repita números brutos.',
    'Use Português-BR impecável. NUNCA use termos corporativos.'
].join(' ');

const SYSTEM_PROSECUTOR = [
    'Você é o PROMOTOR TÁTICO do Tribunal do Protocolo V.',
    'Sua função é analisar a performance do jogador PELA PERSPECTIVA DO TIME ADVERSÁRIO.',
    'Mostre onde o jogador foi explorado. Identifique matchups desfavoráveis e falhas adaptativas.',
    'Se o jogador superou a adversidade, reconheça — mas explique O QUE o rival tentou.',
    'Responda APENAS em JSON válido. NUNCA repita números brutos.',
    'Use Português-BR impecável. NUNCA use termos corporativos.'
].join(' ');

const SYSTEM_ARBITER = [
    'Você é o ÁRBITRO SUPREMO do Tribunal Tático do Protocolo V.',
    'Você recebe DUAS análises (Advogado e Promotor) da mesma partida e deve julgar qual é mais fundamentada.',
    'Você possui uma BASE DE CONHECIMENTO com padrões aprendidos em análises anteriores.',
    'Sintetize o VEREDITO FINAL combinando os melhores argumentos de cada persona.',
    'Identifique PADRÕES NOVOS que você observou nesta partida para enriquecer sua base.',
    'Responda APENAS em JSON válido. Use Português-BR impecável.'
].join(' ');

/**
 * Constrói prompt para o Advogado (Persona 1).
 */
...
REGRAS:
1. Identifique onde ALIADOS falharam em dar suporte (trades não feitos, rotações tardias)
2. Identifique onde o jogador COMPENSOU falhas do time
3. Se o jogador falhou mesmo com bom suporte, reconheça honestamente
4. CRUZE dados do jogador com os aliados — não analise isoladamente
...
/**
 * Constrói prompt para o Promotor (Persona 2).
 */
...
JSON OBRIGATÓRIO:
{
    ...
    "pontos_fortes": ["Resistência/superação contra adversidade 1", "Mérito 2"],
    "pontos_fracos": ["Exploração sofrida 1", "Vulnerabilidade exposta 2"],
    "matchups_chave": "Principais duelos de agente e quem venceu",
    "nota_prosecutor": "0.0 a 10.0"
}`;
}

/**
 * Constrói prompt para o Árbitro (Persona 3).
 */
function buildArbiterPrompt(advocateInsight, prosecutorInsight, analysisResult, knowledgeBase) {
    ...
    // Formata base de conhecimento para o prompt
    let knowledgeBlock = 'Nenhum padrão anterior registrado (primeira análise).';
    if (knowledgeBase && knowledgeBase.length > 0) {
        knowledgeBlock = knowledgeBase.map(k =>
            `  • [${k.pattern_type}] ${k.context_key} (confiança: ${Math.round(k.confidence * 100)}%, visto ${k.times_seen}x): ${k.observation}`
        ).join('\n');
    }

    return `Você recebe DUAS análises da mesma partida de ${agentName} (${role}) em ${mapName}.
Julgue qual é mais fundamentada e sintetize o veredito final.

=== ANÁLISE DO ADVOGADO (perspectiva aliada) ===
${JSON.stringify(advocateInsight, null, 2)}

=== ANÁLISE DO PROMOTOR (perspectiva rival) ===
${JSON.stringify(prosecutorInsight, null, 2)}

=== DADOS BRUTOS DE REFERÊNCIA ===
Performance Index: ${analysisResult.performance_index} | Rank: ${analysisResult.technical_rank}
K/D: ${analysisResult.kd} | ADR: ${analysisResult.adr} | KAST: ${analysisResult.kast || '?'}%
First Bloods: ${analysisResult.first_kills} | Clutches: ${analysisResult.clutches}
Resultado: ${analysisResult.result} | Mapa: ${mapName}

=== SUA BASE DE CONHECIMENTO (padrões anteriores) ===
${knowledgeBlock}

SUAS OBRIGAÇÕES:
1. Avalie qual análise é mais FUNDAMENTADA nos dados reais (não na retórica)
2. Identifique afirmações SEM SUPORTE de dados em cada análise
3. Sintetize o VEREDITO FINAL combinando os melhores argumentos
4. Identifique PADRÕES NOVOS desta partida para sua base de conhecimento
   - Tipos válidos: agent_matchup, map_tendency, economy_pattern, role_interaction, team_comp
5. A nota final deve refletir SEU julgamento, não a média das notas anteriores

JSON OBRIGATÓRIO:
{
    "diagnostico_principal": "Veredito final sintetizado",
    "pontos_fortes": ["Ponto forte validado pelo tribunal"],
    "pontos_fracos": ["Ponto fraco confirmado pelo tribunal"],
    "nota_coach": "0.0 a 10.0",
    "analise_vencedora": "advocate" | "prosecutor" | "empate",
    "razao_escolha": "Por que esta análise foi considerada superior",
    "padroes_aprendidos": [
        {
            "pattern_type": "tipo_do_padrao",
            "context_key": "chave_contexto_especifica",
            "observation": "O que foi observado nesta partida"
        }
    ]
}`;
}
...
export async function loadArbiterKnowledge(agentName, mapName, enemyAgents = []) {
    ...
    try {
        ...
        // Busca os top 15 padrões por confidence que contenham algum contexto relevante
