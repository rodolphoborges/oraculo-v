...
// Modelos para cada persona — Ordem de tentativa: Groq (Preferencial) -> OpenRouter Free (Lista de resiliência)
...
const SYSTEM_ADVOCATE = [
    'Você é o ANALISTA DE SINERGIA de First Person Shooter(FPS) VALORANT.',
...
const SYSTEM_PROSECUTOR = [
    'Você é o ANALISTA RIVAL de First Person Shooter(FPS) VALORANT.',
...
const SYSTEM_ARBITER = [
    'Você é o K.A.I.O., MESTRE TÁTICO E MENTOR SUPREMO de First Person Shooter(FPS) VALORANT.',
...
FALHA: ${roleObligation.failure_indicators}`;
    ...
    return `Você está analisando a partida de ${playerStats?.player_id || 'jogador'} (${agentName}, ${role}). Avalie pela perspectiva do TIME ALIADO.
...
async function buildProsecutorPrompt(matchData, analysisResult, teamContext) {
    ...
    return `Você está analisando a partida de ${playerStats?.player_id || 'jogador'} (${agentName}, ${role}). Avalie pela perspectiva do TIME ADVERSÁRIO.
...
REGRAS:
1. Fale DIRETAMENTE com o jogador: "Você foi lido...", "Te neutralizaram quando..."
2. Identifique quais agentes rivais EXPLORARAM fraquezas do jogador
3. Analise matchups de agente: ${agentName} vs cada agente rival — quem levou vantagem?
4. Identifique rounds/situações onde a composição rival NEUTRALIZOU o jogador
5. Se o jogador resistiu bem à pressão adversária, reconheça — mas explique O QUE o rival tentou
6. NUNCA atribua habilidades que ${agentName} não possui
7. NUNCA mencione sites que não existem neste mapa
8. NUNCA use termos como "O jogador", "O agente", "A performance" — fale com ELE
 ...
 * Cascata: Local → Groq → OpenRouter (Lista de Modelos Gratuitos)
 */
async function callPersona(systemPrompt, userPrompt, personaName, mapName, agentName) {
    ...
    if (LOCAL_URL && LOCAL_MODEL) {
        try {
            ...
            const tid = setTimeout(() => controller.abort(), 60000); // 1 min timeout local (mais agressivo)
            ...
            console.warn(`⚠️ [TRIBUNAL/${personaName.toUpperCase()}] Local falhou ou indisponível: ${e.message}`);
        }
    }

    const personaConfig = PERSONA_MODELS[personaName];

    // 2. GROQ
    if (GROQ_API_KEY && personaConfig?.groq) {
        for (const groqModel of personaConfig.groq) {
            try {
                const controller = new AbortController();
                ...
                console.warn(`⚠️ [TRIBUNAL/${personaName.toUpperCase()}] Groq (${groqModel}) falhou: ${e.message}`);
            }
        }
    }

    // 3. OPENROUTER (Fallback Final com Lista de Gratuitos)
    if (OPENROUTER_KEY && personaConfig?.openrouter) {
        for (const orModel of personaConfig.openrouter) {
            try {
                console.log(`📡 [TRIBUNAL/${personaName.toUpperCase()}] Tentando OpenRouter Free: ${orModel}`);
                const controller = new AbortController();
                ...
                console.warn(`⚠️ [TRIBUNAL/${personaName.toUpperCase()}] OpenRouter (${orModel}) falhou: ${e.message}`);
            }
        }
    }
...
export async function updateArbiterKnowledge(patterns, matchId) {
    ...
    for (const pattern of patterns) {
        ...
        // Normaliza context_key: lowercase, remove pontuação espúria, colapsa separadores
        const key = pattern.context_key
            .toLowerCase()
            .replace(/[^a-z0-9_\s]/g, '')   // remove pontuação (ex: ponto em "jett_vs._raze")
            .replace(/\s+/g, '_')
            .replace(/_+/g, '_')             // colapsa underscores duplos
            .replace(/^_|_$/g, '');           // remove underscores nas pontas

        // Valida pattern_type contra whitelist
        const VALID_PATTERN_TYPES = ['agent_matchup', 'map_tendency', 'economy_pattern', 'role_interaction', 'team_comp'];
        const type = pattern.pattern_type.toLowerCase();
        if (!VALID_PATTERN_TYPES.includes(type)) {
            console.warn(`⚠️ [ARBITER-KB] pattern_type inválido "${type}" para key "${key}" — ignorando.`);
            continue;
        }

        // Valida que observation está em PT-BR (rejeita textos majoritariamente em inglês)
        const englishWords = (pattern.observation.match(/\b(the|is|are|was|were|can|will|with|that|this|from|have|has|been|for|not|but|which|when|their|into|also|than|its|over|such|after|should|would|could|these|those|being|between|during|without|against|through|leading)\b/gi) || []);
        if (englishWords.length >= 5) {
            console.warn(`⚠️ [ARBITER-KB] Observation em inglês detectada para "${key}" — ignorando.`);
            continue;
        }
