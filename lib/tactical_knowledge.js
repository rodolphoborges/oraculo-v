...
export function getGlobalStrategicSummary() {
    let summary = "=== BANCO DE DADOS TÁTICO PROTOCOLO V ===\n\n";

    // 1. Funções e Obrigações
    summary += "--- DIRETRIZES POR FUNÇÃO ---\n";
    for (const [role, data] of Object.entries(ROLE_OBLIGATIONS)) {
        summary += `[${role.toUpperCase()}]: Métrica Principal: ${data.primary_metric} | Obrigações: ${data.obligations.slice(0, 3).join(', ')}\n`;
    }

    // 2. Agentes e Habilidades (Compacto)
    summary += "\n--- AGENTES E CAPACIDADES ---\n";
    for (const [id, a] of Object.entries(AGENTS)) {
        const syn = AGENT_SYNERGY[id] ? ` | Forte com: ${AGENT_SYNERGY[id].strong_with.join(', ')}` : "";
        summary += `${a.name} (${a.role}): ${Object.values(a.abilities).flat().join(', ')}${syn}\n`;
    }

    // 3. Meta de Mapas
    summary += "\n--- META ESTRATÉGICO DE MAPAS ---\n";
    for (const [map, data] of Object.entries(MAP_META)) {
        summary += `${map.toUpperCase()}: Agentes Fortes: ${data.strong_agents.join(', ')} | Estilo: ${data.style}\n`;
    }
