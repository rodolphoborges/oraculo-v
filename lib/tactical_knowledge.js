...
export function calculatePerformanceScore(stats, role) {
    ...
    const w = PERFORMANCE_WEIGHTS[normalizedRole] || PERFORMANCE_WEIGHTS.default;
    
    // Normalização baseada no modelo ImpactAnalyzer V2
    const nADR = Math.min(100, ((stats.adr || 0) / 220) * 100);
    const nKAST = Math.min(100, stats.kast || 0);
    const nFB = Math.min(100, ((stats.first_bloods || 0) / 6) * 100);
    const nClutch = Math.min(100, ((stats.clutches || 0) / 3) * 100);
    const nACS = Math.min(100, ((stats.acs || 0) / 350) * 100);

    const score = (w.adr * nADR) + (w.kast * nKAST) + (w.fb * nFB) + (w.clutch * nClutch) + (w.acs * nACS);
...
export const ROLE_OBLIGATIONS = {
    duelista: {
        ...
        secondary_metrics: "First Deaths (FD) — deve ser mantido sob controle relativo ao FB.",
        obligations: [
            "Abrir rounds com First Blood",
            "Entrar no site PRIMEIRO sob proteção",
            "Manter ADR alto (pressão)",
            "Garantir kills de entrada (entry fragger)"
        ],
        success_indicators: "ADR ≥150, ≥3 FB/partida, K/D positivo",
        failure_indicators: "ADR <110, 0 FB, entrar por último no bomb",
        coach_must_check: "Eficiência de abertura: Quantidade de FB comparado com FD.",
        do_not_penalize: "Baixo KAST em rounds curtos onde o time morre rápido demais."
    },
    iniciador: {
        primary_metric: "KAST e Assistências",
        secondary_metrics: "Assistências por utilitário e Danos assistidos.",
        obligations: [
        ...
        success_indicators: "KAST ≥72%, assistências consistentes, ADR equilibrado",
        failure_indicators: "KAST <60%, surdez tática no recon (não usa flash p/ time)",
        coach_must_check: "Timing de utilitários: As utilidades garantiram kills pros aliados?",
        do_not_penalize: "Baixa taxa de First Bloods (Duelo inicial não é sua função principal)."
    },
    controlador: {
        primary_metric: "KAST e Clutches",
        secondary_metrics: "Duração de smokes e mortes após tempo de smoke.",
        obligations: [
            "Domínio de visão com smokes estratégicas",
            "Negar retake no pós-plant ou travar entrada no def",
            "Sobreviver para garantir smokes tardias",
            "Clutchar rounds mantendo calma tática"
        ],
        success_indicators: "KAST ≥75%, smokes bem posicionadas, Clutch v1",
        failure_indicators: "KAST <65%, morte precoce constante impedindo smokes",
        coach_must_check: "Sobrevivência: O controlador foi o primeiro a morrer? Isso é falha crítica.",
        do_not_penalize: "Falta de abates agressivos (Fragging de entrada)."
    },
    sentinela: {
        primary_metric: "KAST e Info Defensiva",
        secondary_metrics: "First Deaths e Eficiência de Âncora (sobreviver no bomb).",
        obligations: [
            "Proteger o flanco com armadilhas (informação)",
            "Âncora informacional do round",
            "NUNCA ser o entry frag isolado",
            "Monitorar rotações via sensores"
        ],
        success_indicators: "KAST ≥70%, flanco protegido, abates passivos (pós-trap)",
        failure_indicators: "KAST <60%, flanco exposto por negligência ou morte tola",
        coach_must_check: "Posicionamento: Estava fora de posição no início do round?",
        do_not_penalize: "AUSÊNCIA DE FIRST BLOODS (Duelo inicial agressivo é contra-indicado)."
    }
};
...
export function getKnownAgents() {
    return Object.values(AGENTS).map(a => `${a.name} (${a.role})`);
}

/**
 * Gera um resumo denso de toda a estratégia do Oráculo-V para injeção em LLMs.
 */
export function getGlobalStrategicSummary() {
    let summary = "=== HIERARQUIA TÁTICA ATUALIZADA (Siga isso estritamente) ===\n\n";

    // 1. Agrupar Agentes por Função para evitar confusão do LLM
    const roles = {
        [ROLES.DUELISTA]: [],
        [ROLES.SENTINELA]: [],
        [ROLES.INICIADOR]: [],
    ...
    for (const [id, a] of Object.entries(AGENTS)) {
        const syn = AGENT_SYNERGY[id] ? ` | Forte com: ${AGENT_SYNERGY[id].strong_with.join(', ')}` : "";
        const entry = `- ${a.name}: ${Object.values(a.abilities).flat().join(', ')}${syn}`;
        if (roles[a.role]) roles[a.role].push(entry);
    }

    for (const [roleName, agentList] of Object.entries(roles)) {
        summary += `--- ${roleName.toUpperCase()}S ---\n`;
        summary += `Diretriz: ${ROLE_OBLIGATIONS[roleName]?.primary_metric || 'N/A'}\n`;
        summary += agentList.join('\n') + "\n\n";
    }

    // 2. Meta de Mapas
    summary += "--- META DE MAPAS ---\n";
    for (const [map, data] of Object.entries(MAP_META)) {
        summary += `${map.toUpperCase()}: ${data.strong_agents.join(', ')} (${data.style})\n`;
    }
