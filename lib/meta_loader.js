...
export async function getAgentMeta(agentName, mapName = 'ALL', rankTier = 'ALL') {
  ...
  try {
    ...
    // Silencioso: Se a tabela não existe, usamos baselines padrão
    return null;
  }
}
...
export async function getRankBaselines(agentName, mapName = 'ALL') {
  ...
  const results = await Promise.all(
    ranks.map(rank => getAgentMeta(agentName, mapName, rank))
  );

  const baselines = results
    .filter(meta => meta !== null)
    .map((meta, index) => ({ rank: ranks[index], kd: meta.kd }));

  // Fallback se não encontrar os ranks específicos na base local
  if (baselines.length === 0) {
    baselines.push(
      { rank: 'OURO', kd: 1.00, adr: 135 },
      { rank: 'DIAMANTE', kd: 1.10, adr: 150 },
      { rank: 'RADIANTE', kd: 1.25, adr: 165 }
    );
  }
