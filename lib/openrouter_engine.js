...
export async function generateInsights(promptData) {
    ...
    // 🚀 Última Linha de Defesa: Fallback Local (NAT)
    if (LOCAL_URL && LOCAL_MODEL) {
        console.log(`🏠 [LLM-LOCAL] Acionando Resiliência Física via NAT com: ${LOCAL_MODEL} em ${LOCAL_URL}`);
        try {
            ...
            if (resp.ok) {
                ...
                if (json.choices && json.choices.length > 0) {
                    ...
                    try {
                        ...
                        console.log('✅ [LLM-LOCAL] Insight gerado via Servidor Ollama Privado!');
                        return { insight: parsed, model_used: `local-${LOCAL_MODEL}` };
                    } catch (err) {
                        console.warn('[LLM-LOCAL] O modelo local falhou ao montar um JSON válido.', textOutput);
                    }
                }
            } else {
                console.warn(`[LLM-LOCAL] Ollama falhou com HTTP ${resp.status}`);
            }
        } catch (e) {
            console.error(`❌ [LLM-LOCAL] Falha de conexão com o NAT/Ollama na rede local: ${e.message}`);
        }
    }

    console.error('❌ [CRÍTICO] Falha Completa da Inteligência. Free-Tier e Modelo Local estão offline.');
    return null;
}
