...
// OpenRouter — fallback final (modelos COMPLETAMENTE GRATUITOS)
const FALLBACK_MODELS = [
    'google/gemini-2.0-flash-lite-preview-02-05:free', // O novo padrão ouro para JSON e velocidade (Gratuito)
    'google/gemini-2.0-flash-exp:free',               // Excelente estabilidade (Gratuito)
    'qwen/qwen-2.5-72b-instruct:free',               // Grande modelo chinês muito inteligente (Gratuito)
    'meta-llama/llama-3.1-8b-instruct:free',         // Equilíbrio perfeito (Gratuito)
    'deepseek/deepseek-r1:free',                     // Focado em raciocínio (Gratuito)
    'mistralai/mistral-7b-instruct:free'             // Estabilidade em PT-BR (Gratuito)
];
...
const BANNED_TERMS = [
    ...
    'advogado', 'promotor', 'tribunal', 'julgamento'
];
...
async function buildPrompt(dados_supabase_match, tendencias_query, historico_insights, squad_context) {
    ...
    // Keywords estáticas do Engine Tático 2.0 (abilities + tags)
...
MÉTRICAS SECUNDÁRIAS: ${roleObligation.secondary_metrics || 'N/A'}
OBRIGAÇÕES TÁTICAS:
...
O COACH DEVE FOCAR EM: ${roleObligation.coach_must_check || 'Cruzamento de ADR/KAST'}
⚠️ NÃO PENALIZAR: ${roleObligation.do_not_penalize || 'Ações fora de sua atribuição primária'}`;
    }
    ...
    // --- BLOCO 4: Análise Temporal Cross-Round ---
    const crossRound = dados_supabase_match.cross_round;
    const roundNarrative = dados_supabase_match.round_narrative;
    let temporalBlock = "";
    if (crossRound) {
        ...
        if (h1?.rounds > 0 && h2?.rounds > 0) {
            lines.push(`1º TURNO (Rds 1-12, ${h1.rounds} rounds): K/D ${h1.kills}/${h1.deaths} (${h1.kd}) | ADR ${h1.adr}`);
            lines.push(`2º TURNO (Rds 13+, ${h2.rounds} rounds): K/D ${h2.kills}/${h2.deaths} (${h2.kd}) | ADR ${h2.adr}`);
        }

        const bt = crossRound.by_type || {};
        const typeParts = [];
        if (bt.pistol?.rounds > 0)
            typeParts.push(`Pistola: ${bt.pistol.kills}K/${bt.pistol.deaths}M (${bt.pistol.won}/${bt.pistol.rounds} rounds ganhos)`);
        if (bt.eco?.rounds > 0)
            typeParts.push(`Eco: ${bt.eco.kills}K em ${bt.eco.rounds} rounds`);
        if (bt.force?.rounds > 0)
            typeParts.push(`Force Buy: ${bt.force.kills}K/${bt.force.deaths}M em ${bt.force.rounds} rounds`);
        if (bt.full?.rounds > 0)
            typeParts.push(`Full Buy: ${bt.full.kills}K/${bt.full.deaths}M em ${bt.full.rounds} rounds`);
        if (typeParts.length > 0) lines.push(`Por compra: ${typeParts.join(' | ')}`);

        if (crossRound.trade_rate !== null && crossRound.trade_rate !== undefined)
            lines.push(`Taxa de Trade: ${crossRound.trade_rate}% das mortes sofridas foram tradadas pelo time`);
        if (crossRound.opening_duel_wr !== null && crossRound.opening_duel_wr !== undefined)
            lines.push(`Duelos de Abertura: ${crossRound.opening_duel_wr}% de aproveitamento`);
        if (crossRound.max_pos_streak > 2)
            lines.push(`Maior sequência positiva: ${crossRound.max_pos_streak} rounds consecutivos`);
        if (crossRound.max_neg_streak > 2)
            lines.push(`Maior sequência negativa: ${crossRound.max_neg_streak} rounds consecutivos`);
        if (roundNarrative)
            lines.push(`Linha do tempo (rounds notáveis): ${roundNarrative}`);

        if (lines.length > 0)
            temporalBlock = `\n--- ANÁLISE TEMPORAL (USE PARA CRUZAR COM OS STATS AGREGADOS) ---\n${lines.join('\n')}`;
    }
...
${roleContextBlock}${temporalBlock}
...
2. NUNCA repita os números brutos que já estão nos dados. O jogador já sabe o KDA, ADR e ACS dele. Sua função é CRUZAR as métricas e expor o que elas revelam em conjunto — contradições, padrões ocultos, falhas de execução tática. Se houver ANÁLISE TEMPORAL, use-a para identificar se o jogador decaiu no 2º turno, teve problemas em rounds eco, ou perdeu duelos de abertura sistematicamente.
3. Use Português-BR impecável. NUNCA use termos corporativos (métrica, validação, iteração, alavancagem).
4. NUNCA use inglês desnecessário (feedback, dynamics, missions, subpar).
5. Se rank for Depósito de Torreta, seja direto e cite as falhas táticas específicas derivadas dos dados.
6. Se rank for Alpha, reconheça a execução superior com base nos cruzamentos de dados.
7. Use callouts BR reais do mapa acima — NUNCA invente posições que não existem.
8. NUNCA atribua habilidades que o agente não possui.
9. NUNCA use as frases "TENDÊNCIA POSITIVA" ou "VIOLAÇÃO" no roteiro. O painel estatístico já diz isso. Aja como treinador usando palavras próprias.
10. Cada ponto forte ou fraco deve ser uma CONCLUSÃO derivada dos dados, não uma descrição do que os dados dizem.
...
{
    "diagnostico_principal": "Conclusão tática derivada do cruzamento dos dados — não repita números",
    "pontos_fortes": ["Conclusão não-óbvia sobre execução 1", "Conclusão 2"],
    "pontos_fracos": ["Falha tática revelada pelo cruzamento de dados 1", "Falha 2"],
...
// System prompt compartilhado — analítico, data-driven, sem obviedades
const SYSTEM_PROMPT_TACTICAL = [
    'Você é um analista tático de Valorant. Responda APENAS em JSON válido.',
    'REGRA ABSOLUTA: Baseie-se EXCLUSIVAMENTE nos dados recebidos. Não invente cenários, não suponha contexto que não foi fornecido.',
    'PROIBIDO: Repetir os números que já estão nos dados (o jogador já sabe o KDA dele). Sua função é INTERPRETAR o que os números revelam — cruzamentos, padrões e contradições que não são óbvios.',
    'Exemplo de OBVIEDADE proibida: "O jogador teve 24 kills e 12 deaths". Exemplo de INSIGHT válido: "KD alto mas ADR baixo indica kills de baixo impacto — provavelmente finalizando abates que aliados iniciaram."'
].join(' ');

/**
 * Chama o modelo local (Ollama) e retorna {insight, model_used} ou null.
 * @param {string} sysPrompt - Prompt completo montado pelo buildPrompt
 * @param {string} mapName - Nome do mapa para validação de qualidade
 * @param {string} agentName - Nome do agente para validação de qualidade
 */
        ...
        try {
            ...
            const resp = await fetch(`${LOCAL_URL}/api/chat`, {
                ...
                body: JSON.stringify({
                    ...
                    format: 'json',
                    messages: [
                        { role: 'system', content: SYSTEM_PROMPT_TACTICAL },
...
/**
 * GERAÇÃO ESTRUTURADA COM FALLBACK SEQUENCIAL (Local-First):
 *   1. 🏠 LOCAL (Ollama)  - Tentativa prioritária (Sem custo, baixa latência)
 *   2. ⚡ GROQ (Fallback 1) - Se Local falhar, usa Groq (Rápido e gratuito)
 *   3. 🧠 OPENROUTER (Último caso) - Fallback final sequencial
 */
export async function generateInsights(promptData) {
    ...
    if (cached && (Date.now() - cached.ts) < CACHE_TTL_MS) {
        console.log(`⚡ [CACHE-HIT] Perfil "${cacheKey}" — retornando cached.`);
    ...
    console.log('🏠 [INTELLIGENCE] Tentando motor local (Prioridade 1)...');
    ...
    console.warn('⚠️ [INTELLIGENCE] Motor Local indisponível. Tentando Fallback 1 (Groq)...');
    ...
    // --- [3. ÚLTIMO CASO: OPENROUTER SEQUENCIAL] ---
    console.warn('🚨 [INTELLIGENCE] Fallbacks prioritários falharam. Iniciando recuperação via OpenRouter...');
    for (const model of FALLBACK_MODELS) {
        const result = await callOpenRouter(model, sysPrompt, mapName, agentName);
        if (result) {
            console.log(`✨ [INTELLIGENCE] Recuperação final via: ${model}`);
            _insightCache.set(cacheKey, { result, ts: Date.now() });
            return result;
        }
    }

    console.error('❌ [CRÍTICO] Falha total da Inteligência. Verifique conexão local ou API Keys Cloud.');
    return null;
}
