...
{
    "diagnostico_principal": "Texto curto impactante para o cabeçalho",
    "pontos_fortes": ["Vantagem tática 1", "Vantagem tática 2"],
    "pontos_fracos": ["Falha mecânica/posicionamento 1", "Falha crítica 2"],
...
export async function generateInsights(promptData) {
    ...
    if (LOCAL_URL && LOCAL_MODEL) {
        ...
        if (localModel.includes('--model')) {
            localModel = localModel.split('--model').pop().trim();
        }

        const MAX_RETRIES = 3;
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            console.log(`🏠 [LLM-LOCAL] Tentativa ${attempt}/${MAX_RETRIES}: ${localModel}`);
            try {
                const controller = new AbortController();
                const timeoutId = setTimeout(() => controller.abort(), 120000); // 120s (Exaustivo)
                ...
                if (resp.ok) {
                    ...
                    if (json.message && json.message.content) {
                        let textOutput = json.message.content;
                        ...
                        try {
                            ...
                            if (validateInsightQuality(parsed, mapName, agentName)) {
                                console.log(`✅ [LLM-LOCAL] Insight aprovado na tentativa ${attempt}.`);
                                return { insight: parsed, model_used: `local-${localModel}` };
                            } else {
                                console.warn(`⚠️ [LLM-LOCAL] Tentativa ${attempt} falhou no filtro de qualidade.`);
                            }
                        } catch (err) {
                            console.warn(`[LLM-LOCAL] Falha no parse/limpeza na tentativa ${attempt}.`, err.message);
                        }
                    }
                } else {
                    const errorText = await resp.text();
                    console.error(`❌ [LLM-LOCAL] Erro na resposta (${resp.status}): ${errorText}`);
                }
            } catch (e) {
                console.error(`❌ [LLM-LOCAL] Falha na tentativa ${attempt}: ${e.message}`);
                if (e.name === 'AbortError') console.error('⏰ [LLM-LOCAL] Timeout de 120s atingido.');
            }
    ...
    if (API_KEY) {
        for (let i = 0; i < FALLBACK_MODELS.length; i++) {
            ...
            try {
                // Timeout mais generoso para modelos locais pesados (60s)
                const controller = new AbortController();
