...
1. USE VOCABULÁRIO DE FPS/VALORANT: 'Entry frag', 'Trade/re-frag', 'Lurker', 'Rotate/Rotacionar', 'Crosshair placement', 'Econ/Eco round', 'Full Buy', 'Save', 'Utilities', 'Bomb site', 'Peak/Wide peak', 'Hold/Segurar'.
2. NUNCA USE ESTES TERMOS (TRADUÇÕES ERRADAS): 'menear', 'preservar a time', 'precisão nas assists', 'Pratele', 'alinharmos', 'instâncias'. Use termos de jogo reais.
3. FOCO NO PAPEL: Avalie se o agente escolhido cumpriu a sua métrica primária (Duelista = FB, Controlador = Fumaça/Trade, Iniciador = Assist/KAST, Sentinela = Lock/Antiflank).
4. ANÁLISE DE MOMENTUM: Verifique se o jogador está numa "Win Streak" ou em "Lose Streak".
5. ECONOMIA/UTILIDADE: Puna verbalmente gastos desnecessários ou falta de uso de habilidades em rodadas decisivas.

RESPOSTA OBRIGATÓRIA (Formato JSON puro):
{
    "diagnostico_principal": "Texto curto brutal com termos técnicos",
    "foco_treino": ["Dica técnica 1", "Dica técnica 2"],
            ...
            if (resp.ok) {
                ...
                if (json.choices && json.choices.length > 0) {
                    ...
                    try {
                        ...
                        console.log('✅ [LLM-LOCAL] Insight gerado com Sucesso!');
                        return { insight: parsed, model_used: `local-${LOCAL_MODEL}` };
                    } catch (err) {
                        console.warn('[LLM-LOCAL] O modelo local falhou ao montar um JSON válido. Tentando Fallback OpenRouter...', textOutput);
                    }
                }
            } else {
